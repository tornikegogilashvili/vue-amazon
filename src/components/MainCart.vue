...
<button :count="this.$store.state.productToCart.length" :class="['btn btn-warning d-block ms-0 ms-lg-2 position-relative',{'mybtn':this.$store.state.productToCart.length>0}]"
    ...
    <Modal bsTarget="cart" :modalTitle="$t('cart')">
          ...
          <table class="table" v-if="this.$store.state.productToCart.length>0">
            ...
            <tbody>
                <tr v-for="product in this.$store.state.productToCart" :key="product.id">
                    ...
                    <td class="fs-5 fw-bold text-danger cursor-pointer" @click="deleteFromCart(product.id)">X</td>
                </tr>
            </tbody>
          </table>
